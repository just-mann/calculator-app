...
function appendDecimalNumbers(number) {
    if (shouldRese) resetScreen();
    if (showRe.textContent === "") showRe.textContent = "0";
    if (showRe.textContent.includes(".")) return;
//   screen.textContent += ".";
    showResult.textContent += number;
}
...
function evaluate() {
    ...
    roundResult(showResult.textContent = operate(currentOperation, firstOperand, secondOperand));
