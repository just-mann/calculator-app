...
numberButtons.forEach(button => {
    button.addEventListener('click', () => {
        appendNumber(button.inne);
    })
})

function appendNumber(number) {
    if (showResult.inne === '0' || shouldReset) {
            showResult.inne = "";
            shouldReset = false;
    }
    showResult.inne += number;
}

// When operator buttons are clicked, run the setOperation function
operationSigns.forEach(sign => {
    sign.addEventListener('click', (e) => {
        setOperation(e.target.inne)
    });
});
...
function evaluate() {
    ...
    if (currentOperation === '/' && showResult.inne === '0') {
        alert('Cannot divide by zero');
        // clearField() 
        return;
    }
    secondOperand = showResult.inne;
    showResult.inne = operate(currentOperation, firstOperand, secondOperand);
